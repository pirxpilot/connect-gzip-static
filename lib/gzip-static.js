...
function gzFileExists(fileName, gzipCache, options) {
  if(options.gzCached) {
    return !!gzipCache[fileName];
  } else {
...
module.exports = function(root, options) {
  var serveStatic, gzipCache = null;

  options = options || {gzCached: true};
  options.index = options.index || 'index.html';
  serveStatic = require('serve-static')(root, options);

  if(options.gzCached === true) {
  ...
  return function gzipStatic(req, res, next) {
    ...
    if (!gzFileExists(name.full, gzipCache, options)) {
      debug('Passing %s', req.url);
      return passToStatic();
    }
